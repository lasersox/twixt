...
def train(number_of_games=100, search_depth=2):
    ...
    c1 = ComputerPlayer("muzi", copy.deepcopy(weights), search_depth = 2, learning_rate=0.04)
    ...
    for n in range(number_of_games):
        ...
        while True:
            ...
            if teacher.name == game.current_player and len(score_buffer1) == search_depth/2:
                # print "Updating trainee..."
                expected_score1 = score_buffer1.pop(0)
                actual_score1 = teacher.get_score(game)
                score_buffer1.append(actual_score1)
                effs = [eval('heuristic.f_'+str(i)+'(game, players[game.current_player])') for i in range(1,11)]
                old_weights = copy.deepcopy(teacher.weights)
                teacher.update_weights(expected_score1, actual_score1, effs)
                # print [old_weights[i] - trainee.weights[i] for i in range(len(trainee.weights))]
            elif teacher.name == game.current_player and len(score_buffer1) != search_depth/2:
                actual_score1 = teacher.get_score(game)
                score_buffe1r.append(actual_score1)
